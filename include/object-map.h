...
namespace SoftDocument {
  template <typename Document, typename Value, typename Symbol, bool orderedMap>
  struct ObjectMap;

  template <typename Document, typename Value, typename Symbol>
  struct Property {
    Property* next;
    Symbol* key;
    Value value;
  ...
  template <typename Document, typename Value, typename Symbol>
  struct ObjectMap<Document, Value, Symbol, true> : Object {
      ...
      for (Property *propX = this->first; propX; propX = propX->next) {
        ...
        if (propY) {
          ...
          if (diff.typeID != TypeID::Undefined) {
            ...
            Property* prop = obj->map(propX->key, document);
            prop->value.set(&diff);
          }
        }
        else {
    ...
    Property* map(const char* symbol, size_t symbolLen, Document* document) {
      return this->map(Symbol::hash_symbol(symbol, symbolLen), symbol, symbolLen, document);
    }
    ...
    Property* map(Symbol* key, Document* document) {
    ...
    Property* find(Symbol* key) {
      for (Property* cprop = this->first; cprop; cprop = cprop->next) {
        int c = cprop->key->compare(key);
        if (cprop->key == key) return cprop;
      }
      return 0;
    }
    void print() {
  ...
  template <typename Document, typename Value, typename Symbol>
  struct ObjectMap<Document, Value, Symbol, false> : Object {
    ...
    ObjectMap* subtract(ObjectMap* other, Document* document) {
      ...
      while (propX && propY) {
        ...
        if (!c) {
          ...
          if (diff.typeID != TypeID::Undefined) {
            if (!obj) obj = document->createObjectMap();
            prop = obj->map(propX->key, document);
            prop->value.set(&diff);
          }
